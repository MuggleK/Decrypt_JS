...
var params = {
    document:{
        cookie : "__snaker__captcha=ZuitH8aaBAzxqDTj; _qdda=3-1.1; _qddab=3-bw0gxy.kd8kd3dj"
    },
    location:{
        hash: "",
        host: "dun.163.com",
...
// 本JS的坑： node.js下能出结果但程序不停止。猜测是settimeout的问题，但全局搜到的并不是关键点。经过全局分析，window[u[77]]中的u[77]意为settimeout
// 删除即可；

// 浏览器指纹需要随机选定。可构造userAgent，cookie池
...
function get_fp_cb() {
    // fp -> 浏览器指纹，需随机指定浏览器各个属性
    var dict = {}
    dict['fp'] = window.gdxidpyhxde;
    dict['cb'] = window.key_(8).eypt(window.key_(1).uuid(32));
    return dict;
}

//console.log(get_fp_cb())
//console.log(new Date().getTime() - start)



var traceData = [],pointsStack = []
function random(min, max) {
    return Math.floor(Math.random() * (max - min + 1)) + min;
}

function encryptPoint(token,points) {
    // 轨迹加密
    var click = [];
    var move = [];
    (function () {
    ...
    for (var i = 0; i < move.length; i++) {
        traceData.push(window.key_(8).xor_encode(token, move[i]))
    }
    for (var j = 0; j < click.length; j++) {
        pointsStack.push(window.key_(8).xor_encode(token, click[j]));
    }
    console.log(move)
...
function get_result(token,points) {
    encryptPoint(token,points)
    var data = {};
    data['d'] = '',
        data['m'] = window.key_(8).eypt(window.key_(1).sample(traceData, 50).join(":")),
        data['p'] = window.key_(8).eypt(pointsStack.join(":")),
        data['ext'] = window.key_(8).eypt(window.key_(8).xor_encode(token, pointsStack.length + "," + traceData.length))
